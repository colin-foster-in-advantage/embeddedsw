 ...
 * @return	None.
 *
 * @note		The DataBuffer is circular.  If full, new events will
 ...
 * @return	The log data.
 *
 * @note		The read is "destructive" in the sense an event may only
 *be read once, the read pointer then advancing to the next entry. After reading
 *all the events (see routine XVprocSs_LogDisplay), the log will be empty.
 *
 ******************************************************************************/
...
void XVprocSs_LogDisplay(XVprocSs* InstancePtr) {
    ...
    const char topo_name[XVPROCSS_TOPOLOGY_NUM_SUPPORTED][32] = {
        "Scaler-only", "Full Fledged",    "Deinterlacer-only",
        "Csc-only",    "VCResample-only", "HCResample-only"};
        ...
        switch (Evt) {
            ...
            case (XVPROCSS_EVT_INIT_RESAXIS):
                switch (Data) {
                    case (XVPROCSS_EDAT_CFABSENT):
                        xil_printf(
                            "Error: Reset_AxiS Config struct not found\r\n");
                        break;
                    case (XVPROCSS_EDAT_BADADDR):
                        xil_printf(
                            "Error: Reset_AxiS base address invalid\r\n");
                        break;
            ...
            case (XVPROCSS_EVT_INIT_RESAXIM):
                switch (Data) {
                    case (XVPROCSS_EDAT_CFABSENT):
                        xil_printf(
                            "Error: Reset_AxiMM Config struct not found\r\n");
                        break;
                    case (XVPROCSS_EDAT_BADADDR):
                        xil_printf(
                            "Error: Reset_AxiMM base address invalid\r\n");
                        break;
                    case (XVPROCSS_EDAT_INITFAIL):
            ...
            case (XVPROCSS_EVT_CFG_CSC):
                switch (Data) {
                    ...
                    case (XVPROCSS_EDAT_NO422):
                        xil_printf(
                            "Csc error: 422 color format not allowed\r\n");
                        break;
                    case (XVPROCSS_EDAT_NO420):
                        xil_printf(
                            "Csc error: 420 color format not allowed\r\n");
                        break;
                    case (XVPROCSS_EDAT_VMDIFF):
                        xil_printf(
                            "Csc error: Input & Output Video Mode "
                            "different\r\n");
                        break;
                    case (XVPROCSS_EDAT_HDIFF):
                        xil_printf(
                            "Csc error: Input & Output H Active different\r\n");
                        break;
                    case (XVPROCSS_EDAT_VDIFF):
                        xil_printf(
                            "Csc error: Input & Output V Active different\r\n");
                        break;
            ...
            case (XVPROCSS_EVT_CFG_DEINT):
                switch (Data) {
                    case (XVPROCSS_EDAT_CFABSENT):
                        xil_printf(
                            "Error: Deinterlacer Config struct not found\r\n");
                        break;
                    case (XVPROCSS_EDAT_BADADDR):
                        xil_printf(
                            "Error: Deinterlacer base address invalid\r\n");
                        break;
                    case (XVPROCSS_EDAT_INITFAIL):
                    ...
                    case (XVPROCSS_EDAT_INTPRG):
                        xil_printf(
                            "Deinterlacer error: Input must be interlaced, "
                            "Output must be progressive\r\n");
                        break;
                    case (XVPROCSS_EDAT_CDIFF):
                        xil_printf(
                            "Deinterlacer error: Input & Output Color Format "
                            "different\r\n");
                        break;
                    case (XVPROCSS_EDAT_HDIFF):
                        xil_printf(
                            "Deinterlacer error: Input & Output H Active "
                            "different\r\n");
                        break;
                    case (XVPROCSS_EDAT_VALID):
                        xil_printf(
                            "Info: Deinterlacer-only configuration is "
                            "valid\r\n");
                        break;
                    case (XVPROCSS_EDAT_IPABSENT):
            ...
            case (XVPROCSS_EVT_CFG_HSCALER):
                switch (Data) {
                    case (XVPROCSS_EDAT_CFABSENT):
                        xil_printf(
                            "Error: HScaler Config struct not found\r\n");
                        break;
                    case (XVPROCSS_EDAT_BADADDR):
                    ...
                    case (XVPROCSS_EDAT_IODIFF):
                        xil_printf(
                            "Scaler error: Input & Output Color Format "
                            "different\r\n");
                        break;
                    case (XVPROCSS_EDAT_NO422):
                        xil_printf(
                            "Scaler error: 422 color format not allowed\r\n");
                        break;
                    case (XVPROCSS_EDAT_VALID):
                        xil_printf(
            ...
            case (XVPROCSS_EVT_CFG_VSCALER):
                switch (Data) {
                    case (XVPROCSS_EDAT_CFABSENT):
                        xil_printf(
                            "Error: VScaler Config struct not found\r\n");
                        break;
                    case (XVPROCSS_EDAT_BADADDR):
            ...
            case (XVPROCSS_EVT_CFG_HCR):
                switch (Data) {
                    case (XVPROCSS_EDAT_CFABSENT):
                        xil_printf(
                            "Error: HCResampler Config struct not found\r\n");
                        break;
                    case (XVPROCSS_EDAT_BADADDR):
                        xil_printf(
                            "Error: HCResampler base address invalid\r\n");
                        break;
                    case (XVPROCSS_EDAT_INITFAIL):
                    ...
                    case (XVPROCSS_EDAT_CFIN):
                        xil_printf(
                            "HCResampler error: Video Input must be YUV422 or "
                            "YUV444\r\n");
                        break;
                    case (XVPROCSS_EDAT_CFOUT):
                        xil_printf(
                            "HCResampler error: Video Output must be YUV422 or "
                            "YUV444\r\n");
                        break;
                    case (XVPROCSS_EDAT_VMDIFF):
                        xil_printf(
                            "HCResampler error: Input & Output Video Mode "
                            "different\r\n");
                        break;
                    case (XVPROCSS_EDAT_HDIFF):
                        xil_printf(
                            "HCResampler error: Input & Output H Active "
                            "different\r\n");
                        break;
                    case (XVPROCSS_EDAT_VDIFF):
                        xil_printf(
                            "HCResampler error: Input & Output V Active "
                            "different\r\n");
                        break;
                    case (XVPROCSS_EDAT_VALID):
                        xil_printf(
                            "Info: HCResampler-only configuration is "
                            "valid\r\n");
                        break;
                    case (XVPROCSS_EDAT_IPABSENT):
            ...
            case (XVPROCSS_EVT_CFG_VCRI):
                switch (Data) {
                    case (XVPROCSS_EDAT_CFABSENT):
                        xil_printf(
                            "Error: VCResamplerIn Config struct not found\r\n");
                        break;
                    case (XVPROCSS_EDAT_BADADDR):
                        xil_printf(
                            "Error: VCResamplerIn base address invalid\r\n");
                        break;
                    case (XVPROCSS_EDAT_INITFAIL):
                    ...
                    case (XVPROCSS_EDAT_CFIN):
                        xil_printf(
                            "VCResamplerIn error: Video Input must be YUV420 "
                            "or YUV422\r\n");
                        break;
                    case (XVPROCSS_EDAT_CFOUT):
                        xil_printf(
                            "VCResamplerIn error: Video Output must be YUV420 "
                            "or YUV422\r\n");
                        break;
                    case (XVPROCSS_EDAT_VMDIFF):
                        xil_printf(
                            "VCResamplerIn error: Input & Output Video Mode "
                            "different\r\n");
                        break;
                    case (XVPROCSS_EDAT_HDIFF):
                        xil_printf(
                            "VCResamplerIn error: Input & Output H Active "
                            "different\r\n");
                        break;
                    case (XVPROCSS_EDAT_VDIFF):
                        xil_printf(
                            "VCResamplerIn error: Input & Output V Active "
                            "different\r\n");
                        break;
                    case (XVPROCSS_EDAT_VALID):
                        xil_printf(
                            "Info: VCResampler-only configuration is "
                            "valid\r\n");
                        break;
                    case (XVPROCSS_EDAT_IPABSENT):
                    ...
                    case (XVPROCSS_EDAT_LDCOEF):
                        xil_printf(
                            "Info: VCResamplerIn coefficients loaded\r\n");
                        break;
                }
                break;
            case (XVPROCSS_EVT_CFG_VCRO):
                switch (Data) {
                    case (XVPROCSS_EDAT_CFABSENT):
                        xil_printf(
                            "Error: VCResamplerOut Config struct not "
                            "found\r\n");
                        break;
                    case (XVPROCSS_EDAT_BADADDR):
                        xil_printf(
                            "Error: VCResamplerOut base address invalid\r\n");
                        break;
                    case (XVPROCSS_EDAT_INITFAIL):
                    ...
                    case (XVPROCSS_EDAT_LDCOEF):
                        xil_printf(
                            "Info: VCResamplerOut coefficients loaded\r\n");
                        break;
                }
                break;
            case (XVPROCSS_EVT_INIT_LBOX):
                switch (Data) {
                    case (XVPROCSS_EDAT_CFABSENT):
                        xil_printf(
                            "Error: LetterBox Config struct not found\r\n");
                        break;
                    case (XVPROCSS_EDAT_BADADDR):
            ...
            case (XVPROCSS_EVT_INIT_VDMA):
                switch (Data) {
                    case (XVPROCSS_EDAT_CFABSENT):
                        xil_printf(
                            "Error: Video DMA Config struct not found\r\n");
                        break;
                    case (XVPROCSS_EDAT_BADADDR):
            ...
            case (XVPROCSS_EVT_CFG_MAX):
                switch (Data) {
                    case (XVPROCSS_EDAT_MAX_TABLEOK):
                        xil_printf(
                            "Info: Full mode - Video Routing Table setup "
                            "OK\r\n");
                        break;
                    ...
                    case (XVPROCSS_EDAT_CFIN):
                        xil_printf(
                            "Error: Full mode - Input color format invalid as "
                            "configured\r\n");
                        break;
                    case (XVPROCSS_EDAT_CFOUT):
                        xil_printf(
                            "Error: Full mode - Output color format invalid as "
                            "configured\r\n");
                        break;
                    case (XVPROCSS_EDAT_MAX_WRWINBAD):
                        xil_printf(
                            "Error: Full mode - VDMA Write Channel Window "
                            "Invalid\r\n");
                        break;
                    case (XVPROCSS_EDAT_MAX_RDWINBAD):
                        xil_printf(
                            "Error: Full mode - VDMA Read Channel Window "
                            "Invalid\r\n");
                        break;
                    case (XVPROCSS_EDAT_MAX_SCALEBAD):
                        xil_printf(
                            "Error: Full mode - Scaling Mode not "
                            "supported\r\n");
                        break;
                }
                break;
            case (XVPROCSS_EVT_CFG_VPSS):
                switch (Data) {
                    ...
                    case (XVPROCSS_EDAT_VPSS_FRDIFF):
                        xil_printf(
                            "Error: Not Full mode, and Input & Output Frame "
                            "Rate different\r\n");
                        break;
                    case (XVPROCSS_EDAT_VPSS_IVRANGE):
                        xil_printf(
                            "Error: Input Stream Resolution out of range "
                            "0...MAX\r\n");
                        break;
                    case (XVPROCSS_EDAT_VPSS_OVRANGE):
                        xil_printf(
                            "Error: Output Stream Resolution out of range "
                            "0...MAX\r\n");
                        break;
                    case (XVPROCSS_EDAT_VPSS_WIDBAD):
                        xil_printf(
                            "Error: Input/Output Width not aligned with "
                            "Samples/Clk \r\n");
                        break;
                    case (XVPROCSS_EDAT_VPSS_RESBAD):
                        xil_printf(
                            "Error: 1 Sample/Clk max resolution is "
                            "4K2K@30Hz\r\n");
                        break;
                    case (XVPROCSS_EDAT_VPSS_WIDODD):
                        xil_printf(
                            "Error: YUV422 stream width must be even\r\n");
                        break;
                    case (XVPROCSS_EDAT_VPSS_SIZODD):
                        xil_printf(
                            "Error: YUV420 input width and height must be "
                            "even\r\n");
                        break;
                    case (XVPROCSS_EDAT_VPSS_NOHCR):
                        xil_printf(
                            "Error: HCResampler required but not found\r\n");
                        break;
                    case (XVPROCSS_EDAT_VPSS_NOVCRI):
                        xil_printf(
                            "Error: VCResampler required at input but not "
                            "found\r\n");
                        break;
                    case (XVPROCSS_EDAT_VPSS_NOVCRO):
                        xil_printf(
                            "Error: VCResampler required at output but not "
                            "found\r\n");
                        break;
                    case (XVPROCSS_EDAT_NO420):
                        xil_printf(
                            "Error: Interlaced YUV420 stream not "
                            "supported\r\n");
                        break;
                    case (XVPROCSS_EDAT_VPSS_NODEIN):
                        xil_printf(
                            "Error: Input interlaced but no Deinterlacer "
                            "found\r\n");
                        break;
                }
                break;
            case (XVPROCSS_EVT_CHK_BASEADDR):
                xil_printf(
                    "Error: Video Frame Buffer base address not set\r\n"
                    "       Use XVprocSs_SetFrameBufBaseaddr() before VPSS "
                    "init\r\n");
                break;
            ...
            case (XVPROCSS_EVT_SET_PIPMODE):
                switch (Data) {
                    ...
                    case (XVPROCSS_EDAT_LBOX_ABSENT):
                        xil_printf(
                            "Error: No PIP window, cannot set background "
                            "color\r\n");
                        break;
                }
                break;
            case (XVPROCSS_EVT_CFGERR_VDMA):
                switch (Data) {
                    case (XVPROCSS_EDAT_VDMA_WRCFG):
                        xil_printf(
                            "Error: VDMA Write channel configuration "
                            "failed\r\n");
                        break;
                    case (XVPROCSS_EDAT_VDMA_RDCFG):
                        xil_printf(
                            "Error: VDMA Read channel configuration "
                            "failed\r\n");
                        break;
                    case (XVPROCSS_EDAT_VDMA_WRADR):
                        xil_printf(
                            "Error: VDMA Write channel set buffer address "
                            "failed\r\n");
                        break;
                    case (XVPROCSS_EDAT_VDMA_RDADR):
                        xil_printf(
                            "Error: VDMA Read channel set buffer address "
                            "failed\r\n");
                        break;
                }
                break;
            case (XVPROCSS_EVT_OPERR_VDMA):
                switch (Data) {
                    case (XVPROCSS_EDAT_VDMA_WRXFR):
                        xil_printf(
                            "Error: VDMA Start write transfer failed\r\n");
                        break;
                    case (XVPROCSS_EDAT_VDMA_RDXFR):
                        xil_printf(
                            "Error: VDMA Start read transfer failed\r\n");
                        break;
                    case (XVPROCSS_EDAT_VDMA_WRRES):
                        xil_printf(
                            "Error: VDMA Write channel reset stuck high\r\n");
                        break;
                    case (XVPROCSS_EDAT_VDMA_RDRES):
                        xil_printf(
                            "Error: VDMA Read channel reset stuck high\r\n");
                        break;
                }
                break;
            default:
