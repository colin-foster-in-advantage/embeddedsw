...
void XV_hscaler_Set_HwReg_PixelRate(XV_hscaler* InstancePtr, u32 Data) {
    ...
    XV_hscaler_WriteReg(InstancePtr->Config.BaseAddress,
                        XV_HSCALER_CTRL_ADDR_HWREG_PIXELRATE_DATA, Data);
}
...
UINTPTR XV_hscaler_Get_HwReg_hfltCoeff_BaseAddress(XV_hscaler* InstancePtr) {
    ...
    return (InstancePtr->Config.BaseAddress +
...
UINTPTR XV_hscaler_Get_HwReg_hfltCoeff_HighAddress(XV_hscaler* InstancePtr) {
    ...
    return (InstancePtr->Config.BaseAddress +
            XV_HSCALER_CTRL_ADDR_HWREG_HFLTCOEFF_HIGH);
}
...
u32 XV_hscaler_Get_HwReg_hfltCoeff_TotalBytes(XV_hscaler* InstancePtr) {
    ...
    return (XV_HSCALER_CTRL_ADDR_HWREG_HFLTCOEFF_HIGH -
            XV_HSCALER_CTRL_ADDR_HWREG_HFLTCOEFF_BASE + 1);
}
...
u32 XV_hscaler_Write_HwReg_hfltCoeff_Words(XV_hscaler* InstancePtr,
    ...
    if ((offset + length) * 4 > (XV_HSCALER_CTRL_ADDR_HWREG_HFLTCOEFF_HIGH -
                                 XV_HSCALER_CTRL_ADDR_HWREG_HFLTCOEFF_BASE + 1))
        return 0;

    for (i = 0; i < length; i++) {
        *(int*)(InstancePtr->Config.BaseAddress +
                XV_HSCALER_CTRL_ADDR_HWREG_HFLTCOEFF_BASE + (offset + i) * 4) =
            *(data + i);
    }
    return length;
}
...
u32 XV_hscaler_Read_HwReg_hfltCoeff_Words(XV_hscaler* InstancePtr,
    ...
    for (i = 0; i < length; i++) {
        *(data + i) = *(int*)(InstancePtr->Config.BaseAddress +
                              XV_HSCALER_CTRL_ADDR_HWREG_HFLTCOEFF_BASE +
                              (offset + i) * 4);
    }
    return length;
...
u32 XV_hscaler_Write_HwReg_hfltCoeff_Bytes(XV_hscaler* InstancePtr,
    ...
    if ((offset + length) > (XV_HSCALER_CTRL_ADDR_HWREG_HFLTCOEFF_HIGH -
                             XV_HSCALER_CTRL_ADDR_HWREG_HFLTCOEFF_BASE + 1))
        return 0;

    for (i = 0; i < length; i++) {
        *(char*)(InstancePtr->Config.BaseAddress +
                 XV_HSCALER_CTRL_ADDR_HWREG_HFLTCOEFF_BASE + offset + i) =
            *(data + i);
    }
    return length;
}
...
UINTPTR XV_hscaler_Get_HwReg_phasesH_V_HighAddress(XV_hscaler* InstancePtr) {
    ...
    return (InstancePtr->Config.BaseAddress +
            XV_HSCALER_CTRL_ADDR_HWREG_PHASESH_V_HIGH);
}
...
u32 XV_hscaler_Get_HwReg_phasesH_V_TotalBytes(XV_hscaler* InstancePtr) {
    ...
    return (XV_HSCALER_CTRL_ADDR_HWREG_PHASESH_V_HIGH -
            XV_HSCALER_CTRL_ADDR_HWREG_PHASESH_V_BASE + 1);
}
...
u32 XV_hscaler_Write_HwReg_phasesH_V_Words(XV_hscaler* InstancePtr,
    ...
    if ((offset + length) * 4 > (XV_HSCALER_CTRL_ADDR_HWREG_PHASESH_V_HIGH -
                                 XV_HSCALER_CTRL_ADDR_HWREG_PHASESH_V_BASE + 1))
        return 0;

    for (i = 0; i < length; i++) {
        *(int*)(InstancePtr->Config.BaseAddress +
                XV_HSCALER_CTRL_ADDR_HWREG_PHASESH_V_BASE + (offset + i) * 4) =
            *(data + i);
    }
    return length;
}
...
u32 XV_hscaler_Write_HwReg_phasesH_V_Bytes(XV_hscaler* InstancePtr,
    ...
    if ((offset + length) > (XV_HSCALER_CTRL_ADDR_HWREG_PHASESH_V_HIGH -
                             XV_HSCALER_CTRL_ADDR_HWREG_PHASESH_V_BASE + 1))
        return 0;

    for (i = 0; i < length; i++) {
        *(char*)(InstancePtr->Config.BaseAddress +
                 XV_HSCALER_CTRL_ADDR_HWREG_PHASESH_V_BASE + offset + i) =
            *(data + i);
    }
    return length;
}
...
void XV_hscaler_InterruptEnable(XV_hscaler* InstancePtr, u32 Mask) {
    ...
    Register = XV_hscaler_ReadReg(InstancePtr->Config.BaseAddress,
                                  XV_HSCALER_CTRL_ADDR_IER);
    XV_hscaler_WriteReg(InstancePtr->Config.BaseAddress,
                        XV_HSCALER_CTRL_ADDR_IER, Register | Mask);
}
...
u32 XV_hscaler_InterruptGetEnabled(XV_hscaler* InstancePtr) {
    ...
    return XV_hscaler_ReadReg(InstancePtr->Config.BaseAddress,
                              XV_HSCALER_CTRL_ADDR_IER);
}
