...
* methodology by implementing most common use-case provided by the VDMA
sub-core.
*
* <pre>
...
static int validateWindowSize(const XVidC_VideoWindow* win,
    ...
    if (((win->StartX > HActive)) || ((win->StartY > VActive)) ||
...
static XVprocSs_ScaleMode GetScalingMode(XVprocSs* XVprocSsPtr) {
    ...
    if (XVprocSs_IsPipModeOn(XVprocSsPtr)) {
        ...
        /* validate window */
        status = validateWindowSize(&win, XVprocSsPtr->VidOut.Timing.HActive,
                                    XVprocSsPtr->VidOut.Timing.VActive);
        if (status != XST_SUCCESS) {
            XVprocSs_LogWrite(XVprocSsPtr, XVPROCSS_EVT_CFG_MAX,
                              XVPROCSS_EDAT_MAX_WRWINBAD);
            return (XVPROCSS_SCALE_NOT_SUPPORTED);
        } else {
            XVprocSs_LogWrite(XVprocSsPtr, XVPROCSS_EVT_CFG_MAX,
                              XVPROCSS_EDAT_MAX_SCALEDN);
    ...
    if (XVprocSs_IsZoomModeOn(XVprocSsPtr)) {
        ...
        /* validate window */
        status = validateWindowSize(&win, StrmInPtr->Timing.HActive,
                                    StrmInPtr->Timing.VActive);
        if (status != XST_SUCCESS) {
            XVprocSs_LogWrite(XVprocSsPtr, XVPROCSS_EVT_CFG_MAX,
                              XVPROCSS_EDAT_MAX_RDWINBAD);
            return (XVPROCSS_SCALE_NOT_SUPPORTED);
        } else {
            XVprocSs_LogWrite(XVprocSsPtr, XVPROCSS_EVT_CFG_MAX,
                              XVPROCSS_EDAT_MAX_SCALEUP);
...
int XVprocSs_BuildRoutingTable(XVprocSs* XVprocSsPtr) {
    ...
    switch (CtxtPtr->ScaleMode) {
        ...
        case XVPROCSS_SCALE_UP:
            if (XVprocSsPtr->VdmaPtr) {
                pTable[index++] =
                    XVPROCSS_SUBCORE_VDMA; /* VDMA is before Scaler */
            }
        ...
        case XVPROCSS_SCALE_DN:
            ...
            if (XVprocSsPtr->VdmaPtr) {
                pTable[index++] =
                    XVPROCSS_SUBCORE_VDMA; /* VDMA is after Scaler */
            }
            break;
        ...
        case XVIDC_CSF_YCRCB_420:
            switch (StrmInPtr->ColorFormatId) {
                case XVIDC_CSF_RGB:
                    ...
                    pTable[index++] = XVPROCSS_SUBCORE_CR_H;  // convert
                                                              // 444->422
                    pTable[index++] =
                        XVPROCSS_SUBCORE_CR_V_OUT;  // convert 422->420
                ...
                case XVIDC_CSF_YCRCB_444:
                    pTable[index++] =
                        XVPROCSS_SUBCORE_CSC;  // picture control in 444
                    pTable[index++] = XVPROCSS_SUBCORE_CR_H;  // convert
                                                              // 444->422
                    pTable[index++] =
                        XVPROCSS_SUBCORE_CR_V_OUT;  // convert 422->420
                ...
                case XVIDC_CSF_YCRCB_420:  // Input was up converted to 422
                    pTable[index++] =
                        XVPROCSS_SUBCORE_CSC;  // picture control in 422
                    pTable[index++] =
                        XVPROCSS_SUBCORE_CR_V_OUT;  // convert 422->420
...
void XVprocSs_ProgRouterMux(XVprocSs* XVprocSsPtr) {
    ...
    XAxisScr_MiPortEnable(XVprocSsPtr->RouterPtr, nextMi,
                          XVPROCSS_AXIS_SWITCH_VIDIN_S0);
...
void XVprocSs_SetupRouterDataFlow(XVprocSs* XVprocSsPtr) {
    ...
    if (XVprocSsPtr->VdmaPtr) {
        switch (CtxtPtr->ScaleMode) {
            case XVPROCSS_SCALE_1_1:
            case XVPROCSS_SCALE_UP:
                XVprocSs_VdmaSetWinToUpScaleMode(XVprocSsPtr,
                                                 XVPROCSS_VDMA_UPDATE_ALL_CH);
                break;

            case XVPROCSS_SCALE_DN:
                XVprocSs_VdmaSetWinToDnScaleMode(XVprocSsPtr,
                                                 XVPROCSS_VDMA_UPDATE_ALL_CH);
                break;
        ...
        switch (pTable[count]) {
            case XVPROCSS_SUBCORE_SCALER_V:
                if (XVprocSsPtr->VscalerPtr) {
                    if (CtxtPtr->ScaleMode == XVPROCSS_SCALE_DN) {
                        ...
                        vsc_HeightOut =
                            ((XVprocSs_IsPipModeOn(XVprocSsPtr))
                                 ? CtxtPtr->WrWindow.Height
                                 : XVprocSsPtr->VidOut.Timing.VActive);
                    } else {
                        /* UpScale mode V Scaler is before H Scaler */
                        vsc_WidthIn = ((XVprocSs_IsZoomModeOn(XVprocSsPtr))
                                           ? CtxtPtr->RdWindow.Width
                                           : CtxtPtr->VidInWidth);
                        vsc_HeightIn = ((XVprocSs_IsZoomModeOn(XVprocSsPtr))
                                            ? CtxtPtr->RdWindow.Height
                                            : CtxtPtr->VidInHeight);
                        vsc_HeightOut = XVprocSsPtr->VidOut.Timing.VActive;
                    }

                    Result = XV_VScalerSetup(
                        XVprocSsPtr->VscalerPtr, vsc_WidthIn, vsc_HeightIn,
                        vsc_HeightOut, XVprocSsPtr->CtxtData.StrmCformat);
            ...
            case XVPROCSS_SUBCORE_SCALER_H:
                if (XVprocSsPtr->HscalerPtr) {
                    ...
                    Result = XV_HScalerSetup(
                        XVprocSsPtr->HscalerPtr, hsc_HeightIn, hsc_WidthIn,
                        hsc_WidthOut, XVprocSsPtr->CtxtData.StrmCformat,
            ...
            case XVPROCSS_SUBCORE_LBOX:
                if (XVprocSsPtr->LboxPtr) {
                    ...
                    XV_LBoxSetActiveWin(XVprocSsPtr->LboxPtr, &lboxWin,
                                        XVprocSsPtr->VidOut.Timing.HActive,
                                        XVprocSsPtr->VidOut.Timing.VActive);

                    /* set background to default color on pipe reset */
                    XV_LboxSetBackgroundColor(
                        XVprocSsPtr->LboxPtr,
            ...
            case XVPROCSS_SUBCORE_CSC:
                if (XVprocSsPtr->CscPtr) {
                    // to set up a new resolution, start with default picture
                    // settings
                    XV_CscSetPowerOnDefaultState(XVprocSsPtr->CscPtr);

                    // set the proper color depth: get it from the vprocss
                    // config
                    ColorDepth =
                        (XVidC_ColorDepth)XVprocSs_GetColorDepth(XVprocSsPtr);
                    XV_CscSetColorDepth(XVprocSsPtr->CscPtr, ColorDepth);

                    // all other picture settings are filled in by
                    // XV_CscSetColorspace
                    Result = XV_CscSetColorspace(
                        XVprocSsPtr->CscPtr, CtxtPtr->CscIn, CtxtPtr->CscOut,
            ...
            case XVPROCSS_SUBCORE_DEINT:
                if (XVprocSsPtr->DeintPtr) {
                    ...
                    XV_deinterlacer_Set_height(
                        &XVprocSsPtr->DeintPtr->Deint,
                        XVprocSsPtr->VidIn.Timing.VActive);

                    // TBD
                    XV_deinterlacer_Set_invert_field_id(
                        &XVprocSsPtr->DeintPtr->Deint, 0);
