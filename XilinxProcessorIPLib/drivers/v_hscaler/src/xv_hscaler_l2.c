 ...
 *peek/poke methodology by implementing most common use-case provided by the
 *sub-core. See xv_hscaler_l2.h for a detailed description of the layer-2 driver
 *
 * <pre>
...
extern const short XV_hscaler_Lanczos2_taps6[XV_HSCALER_MAX_H_PHASES]
                                            [XV_HSCALER_TAPS_6];
extern const short
    XV_hscaler_fixedcoeff_taps6_ScalingRatio1p2[XV_HSCALER_MAX_H_PHASES]
                                               [XV_HSCALER_TAPS_6];
extern const short
    XV_hscaler_fixedcoeff_taps6_ScalingRatio2[XV_HSCALER_MAX_H_PHASES]
                                             [XV_HSCALER_TAPS_6];
extern const short
    XV_hscaler_fixedcoeff_taps6_ScalingRatio3[XV_HSCALER_MAX_H_PHASES]
                                             [XV_HSCALER_TAPS_6];
extern const short
    XV_hscaler_fixedcoeff_taps6_ScalingRatio4[XV_HSCALER_MAX_H_PHASES]
                                             [XV_HSCALER_TAPS_6];
extern const short
    XV_hscaler_fixedcoeff_taps8_ScalingRatio2[XV_HSCALER_MAX_H_PHASES]
                                             [XV_HSCALER_TAPS_8];
extern const short
    XV_hscaler_fixedcoeff_taps8_ScalingRatio3[XV_HSCALER_MAX_H_PHASES]
                                             [XV_HSCALER_TAPS_8];
extern const short
    XV_hscaler_fixedcoeff_taps8_ScalingRatio4[XV_HSCALER_MAX_H_PHASES]
                                             [XV_HSCALER_TAPS_8];
extern const short
    XV_hscaler_fixedcoeff_taps10_ScalingRatio3[XV_HSCALER_MAX_H_PHASES]
                                              [XV_HSCALER_TAPS_10];
extern const short
    XV_hscaler_fixedcoeff_taps10_ScalingRatio4[XV_HSCALER_MAX_H_PHASES]
                                              [XV_HSCALER_TAPS_10];
extern const short
    XV_hscaler_fixedcoeff_taps12_ScalingRatio4[XV_HSCALER_MAX_H_PHASES]
                                              [XV_HSCALER_TAPS_12];
...
static void XV_HScalerSelectCoeff(XV_Hscaler_l2* InstancePtr,
    ...
    XV_HScalerLoadExtCoeff(InstancePtr, numPhases, numTaps, coeff);
...
/*****************************************************************************/
/**
 * This function loads user defined filter coefficients in the scaler
 *coefficient storage
 *
 * @param  InstancePtr is a pointer to the core instance to be worked on.
        ...
        for (s = 0; s < HscPtr->Hsc.Config.PixPerClk; s++) {
            ...
            if (HscPtr->Hsc.Config.PixPerClk == XVIDC_PPC_8) {
                if (s < 4) {
                    ...
                    HscPtr->phasesH_H[x] |=
                        (OutputWriteEn << (10 + ((s - 4) * 11)));
                }
            } else if (HscPtr->Hsc.Config.PixPerClk == XVIDC_PPC_4) {
                HscPtr->phasesH[x] = HscPtr->phasesH[x] | (PhaseH << (s * 10));
                HscPtr->phasesH[x] =
                    HscPtr->phasesH[x] | (arrayIdx << (6 + (s * 10)));
                HscPtr->phasesH[x] =
                    HscPtr->phasesH[x] | (OutputWriteEn << (9 + (s * 10)));
            } else {
                HscPtr->phasesH[x] = HscPtr->phasesH[x] | (PhaseH << (s * 9));
                HscPtr->phasesH[x] =
                    HscPtr->phasesH[x] | (arrayIdx << (6 + (s * 9)));
                HscPtr->phasesH[x] =
                    HscPtr->phasesH[x] | (OutputWriteEn << (8 + (s * 9)));
            }
    ...
    switch (HscPtr->Hsc.Config.PixPerClk) {
        ...
        case XVIDC_PPC_8: {
            ...
            for (i = 0; i < loopWidth; i++) {
                ...
                bits_32_63 |=
                    ((u32)((phaseHData_H & XHSC_MASK_LOW_20BITS)) << 12);
                bits_64_95 =
                    (((u32)(phaseHData_H & XHSC_MASK_LOW_32BITS)) >> 20);
                bits_64_95 |=
                    (((u32)(phaseHData_H >> 32) & XHSC_MASK_LOW_12BITS) << 12);
...
static void XV_HScalerSetCoeff(XV_Hscaler_l2* HscPtr) {
    ...
    for (i = 0; i < num_phases; i++) {
        for (j = 0; j < num_taps; j++) {
            rdIndx = j * 2 + offset;
            val = (HscPtr->coeff[i][rdIndx + 1] << 16) |
                  (HscPtr->coeff[i][rdIndx] & XHSC_MASK_LOW_16BITS);
...
int XV_HScalerSetup(XV_Hscaler_l2* InstancePtr,
     ...
     */
    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid((HeightIn > 0) &&
                      (HeightIn <= InstancePtr->Hsc.Config.MaxHeight));
    Xil_AssertNonvoid((WidthIn > 0) &&
                      (WidthIn <= InstancePtr->Hsc.Config.MaxWidth));
    Xil_AssertNonvoid((WidthOut > 0) &&
                      (WidthOut <= InstancePtr->Hsc.Config.MaxWidth));
...
void XV_HScalerDbgReportStatus(XV_Hscaler_l2* InstancePtr) {
    ...
    ctrl = XV_hscaler_ReadReg(HscPtr->Config.BaseAddress,
...
int XV_HScalerValidateConfig(XV_Hscaler_l2* InstancePtr,
    ...
    if (cformatIn == XVIDC_CSF_YCRCB_422 &&
        !XV_HscalerIs422Enabled(InstancePtr)) {
        return (FALSE);
    }

    if (cformatIn == XVIDC_CSF_RGB && !XV_HscalerIsCscEnabled(InstancePtr) &&
        cformatOut != XVIDC_CSF_RGB) {
        return (FALSE);
    }

    if (cformatIn != XVIDC_CSF_RGB) {
        lo = (XV_HscalerIsCscEnabled(InstancePtr)) ? XVIDC_CSF_RGB
                                                   : XVIDC_CSF_YCRCB_444;
        hi =
            (XV_HscalerIs420Enabled(InstancePtr))
                ? XVIDC_CSF_YCRCB_420
                : ((XV_HscalerIs422Enabled(InstancePtr)) ? XVIDC_CSF_YCRCB_422
                                                         : XVIDC_CSF_YCRCB_444);
