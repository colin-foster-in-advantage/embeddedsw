 ...
 *       not do anything
 ******************************************************************************/
static __inline void XVprocSs_EnableBlock(XGpio* pReset,
                                          u32 channel,
                                          u32 ipBlock) {
    u32 val;
 ...
 *       not do anything
 ******************************************************************************/
static __inline void XVprocSs_ResetBlock(XGpio* pReset,
                                         u32 channel,
                                         u32 ipBlock) {
    u32 val;
...
static void SetPowerOnDefaultState(XVprocSs* XVprocSsPtr) {
    ...
    XVprocSsPtr->CtxtData.PixelWidthInBits =
        XVprocSsPtr->Config.NumVidComponents * XVprocSsPtr->VidIn.ColorDepth;
...
void XVprocSs_Reset(XVprocSs* InstancePtr) {
    ...
    /* Reset All IP Blocks on AXIS interface and wait before doing the aximm
     * reset*/
    ...
    /*
     * Make sure the video IP's are out of reset - IP's cannot be programmed
     * when held in reset. Will cause Axi-Lite bus to lock. Release IP reset -
     * but hold vid_in in reset
     */
    ...
    if (XVprocSs_IsConfigModeMax(InstancePtr)) {
        if (mode == XVPROCSS_ZOOM_WIN) {
            ...
            if (!InstancePtr->VdmaPtr->ReadChannel.HasDRE) {
                ...
                win->StartX = ((win->StartX + AlignStartX - 1) / AlignStartX) *
                              AlignStartX;
                win->Width =
...
static int ValidateCscOnlyConfig(XVprocSs* XVprocSsPtr,
    ...
    //   1) if Vin or Vout are 422, and 422 or 420 are enabled, the case is
    //   allowed 2) if Vin or Vout are 420, and 420 is enabled, the case is
    //   allowed
...
static int SetupModeScalerOnly(XVprocSs* XVprocSsPtr) {
    ...
    if (status == XST_SUCCESS) {
        ...
        XV_VScalerSetup(XVprocSsPtr->VscalerPtr, vsc_WidthIn, vsc_HeightIn,
                        vsc_HeightOut, XVprocSsPtr->VidIn.ColorFormatId);

        XV_HScalerSetup(XVprocSsPtr->HscalerPtr, hsc_HeightIn, hsc_WidthIn,
                        hsc_WidthOut, hsc_ColorFormatIn,
...
static int SetupModeCscOnly(XVprocSs* XVprocSsPtr) {
    ...
    if (status == XST_SUCCESS) {
        ...
        XV_CscSetColorspace(XVprocSsPtr->CscPtr, CscIn, CscOut, StdIn, StdOut,
                            RangeOut);
...
static int SetupModeDeintOnly(XVprocSs* XVprocSsPtr) {
    ...
    if (status == XST_SUCCESS) {
        ...
        XV_DeintSetFieldBuffers(XVprocSsPtr->DeintPtr, CtxtPtr->DeintBufAddr,
...
static int SetupModeVCResampleOnly(XVprocSs* XVprocSsPtr) {
    ...
    if (status == XST_SUCCESS) {
        ...
        XV_VCrsmplSetActiveSize(XVprocSsPtr->VcrsmplrInPtr, CtxtPtr->VidInWidth,
                                CtxtPtr->VidInHeight);
...
static int SetupModeHCResampleOnly(XVprocSs* XVprocSsPtr) {
    ...
    if (status == XST_SUCCESS) {
        ...
        XV_HCrsmplSetActiveSize(XVprocSsPtr->HcrsmplrPtr, WidthOut, HeightOut);
 ...
 * processing path. It will examine the instantiated subsystem configuration
 *mode and the input and output stream configuration. Based on the available
...
void XVprocSs_SetPictureColorStdIn(XVprocSs* InstancePtr,
    ...
    if (InstancePtr->CscPtr) {
        XV_CscSetColorspace(CscPtr, CscPtr->ColorFormatIn,
                            CscPtr->ColorFormatOut, NewVal, CscPtr->StandardOut,
                            CscPtr->OutputRange);
    }
}
...
void XVprocSs_SetPictureColorStdOut(XVprocSs* InstancePtr,
    ...
    if (InstancePtr->CscPtr) {
        XV_CscSetColorspace(CscPtr, CscPtr->ColorFormatIn,
                            CscPtr->ColorFormatOut, CscPtr->StandardIn, NewVal,
                            CscPtr->OutputRange);
    }
}
...
void XVprocSs_SetPictureColorRange(XVprocSs* InstancePtr,
    ...
    if (InstancePtr->CscPtr) {
        XV_CscSetColorspace(CscPtr, CscPtr->ColorFormatIn,
                            CscPtr->ColorFormatOut, CscPtr->StandardIn,
                            CscPtr->StandardOut, NewVal);
    }
}
...
int XVprocSs_SetPictureDemoWindow(XVprocSs* InstancePtr,
    ...
    if ((InstancePtr->CscPtr) &&
        (XV_CscIsDemoWindowEnabled(InstancePtr->CscPtr))) {
        ...
        if ((Win->StartX > width) || (Win->StartY > height) ||
            ((Win->StartX + Win->Width) > width) ||
            ((Win->StartY + Win->Height) > height)) {
            status = XST_FAILURE;
            XVprocSs_LogWrite(InstancePtr, XVPROCSS_EVT_CFG_CSC,
                              XVPROCSS_EDAT_CSC_BADWIN);
        } else {
            status = XST_SUCCESS;
            XVprocSs_LogWrite(InstancePtr, XVPROCSS_EVT_CFG_CSC,
                              XVPROCSS_EDAT_CSC_SETWIN);
        }
...
void XVprocSs_SetPIPBackgroundColor(XVprocSs* InstancePtr,
    ...
    if (InstancePtr->LboxPtr) {
        XV_LboxSetBackgroundColor(InstancePtr->LboxPtr, ColorId,
        ...
        XVprocSs_LogWrite(InstancePtr, XVPROCSS_EVT_SET_PIPMODE,
                          XVPROCSS_EDAT_BGND_SET);
    } else {
        XVprocSs_LogWrite(InstancePtr, XVPROCSS_EVT_SET_PIPMODE,
                          XVPROCSS_EDAT_LBOX_ABSENT);
    }
}
...
void XVprocSs_LoadScalerCoeff(XVprocSs* InstancePtr,
    ...
    switch (CoreId) {
        case XVPROCSS_SUBCORE_SCALER_V:
            if (InstancePtr->VscalerPtr) {
                XV_VScalerLoadExtCoeff(InstancePtr->VscalerPtr, num_phases,
                                       num_taps, Coeff);
                XVprocSs_LogWrite(InstancePtr, XVPROCSS_EVT_CFG_VSCALER,
                                  XVPROCSS_EDAT_LDCOEF);
            } else {
                XVprocSs_LogWrite(InstancePtr, XVPROCSS_EVT_CFG_VSCALER,
                                  XVPROCSS_EDAT_IPABSENT);
            }
        ...
        case XVPROCSS_SUBCORE_SCALER_H:
            if (InstancePtr->HscalerPtr) {
                XV_HScalerLoadExtCoeff(InstancePtr->HscalerPtr, num_phases,
                                       num_taps, Coeff);
                XVprocSs_LogWrite(InstancePtr, XVPROCSS_EVT_CFG_HSCALER,
                                  XVPROCSS_EDAT_LDCOEF);
            } else {
                XVprocSs_LogWrite(InstancePtr, XVPROCSS_EVT_CFG_HSCALER,
                                  XVPROCSS_EDAT_IPABSENT);
...
void XVprocSs_LoadChromaResamplerCoeff(XVprocSs* InstancePtr,
    ...
    switch (CoreId) {
        case XVPROCSS_SUBCORE_CR_H:
            if (InstancePtr->HcrsmplrPtr) {
                XV_HCrsmplrLoadExtCoeff(InstancePtr->HcrsmplrPtr, num_taps,
                                        Coeff);
                XVprocSs_LogWrite(InstancePtr, XVPROCSS_EVT_CFG_HCR,
                                  XVPROCSS_EDAT_LDCOEF);
            } else {
                XVprocSs_LogWrite(InstancePtr, XVPROCSS_EVT_CFG_HCR,
                                  XVPROCSS_EDAT_IPABSENT);
        ...
        case XVPROCSS_SUBCORE_CR_V_IN:
            if (InstancePtr->VcrsmplrInPtr) {
                XV_VCrsmplrLoadExtCoeff(InstancePtr->VcrsmplrInPtr, num_taps,
                                        Coeff);
                XVprocSs_LogWrite(InstancePtr, XVPROCSS_EVT_CFG_VCRI,
                                  XVPROCSS_EDAT_LDCOEF);
            } else {
                XVprocSs_LogWrite(InstancePtr, XVPROCSS_EVT_CFG_VCRI,
                                  XVPROCSS_EDAT_IPABSENT);
        ...
        case XVPROCSS_SUBCORE_CR_V_OUT:
            if (InstancePtr->VcrsmplrOutPtr) {
                XV_VCrsmplrLoadExtCoeff(InstancePtr->VcrsmplrOutPtr, num_taps,
                                        Coeff);
                XVprocSs_LogWrite(InstancePtr, XVPROCSS_EVT_CFG_VCRO,
                                  XVPROCSS_EDAT_LDCOEF);
            } else {
                XVprocSs_LogWrite(InstancePtr, XVPROCSS_EVT_CFG_VCRO,
                                  XVPROCSS_EDAT_IPABSENT);
...
void XVprocSs_ReportSubcoreStatus(XVprocSs* InstancePtr, u32 SubcoreId) {
    ...
    switch (SubcoreId) {
        ...
        case XVPROCSS_SUBCORE_VDMA:
            XVprocSs_VdmaDbgReportStatus(
                InstancePtr, InstancePtr->CtxtData.PixelWidthInBits);
            break;
...
void XVprocSs_ReportSubsystemCoreInfo(XVprocSs* InstancePtr) {
    const char* topology[XVPROCSS_TOPOLOGY_NUM_SUPPORTED] = {
        "Scaler-Only", "Full-Fledged",  "Deint-Only",
        "CSC-Only",    "420To422-Only", "422To444-Only"};
...
void XVprocSs_ReportSubsystemConfig(XVprocSs* InstancePtr) {
    ...
    if (XVprocSs_IsConfigModeMax(InstancePtr)) {
        ...
        while (count < InstancePtr->CtxtData.RtrNumCores) {
            xil_printf(" -> %s",
                       XVprocSsIpStr[InstancePtr->CtxtData.RtngTable[count++]]);
        }
    }
    xil_printf("\r\n");
}
